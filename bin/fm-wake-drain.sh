...
# Reuse fm-guard.sh's existing graced, identity-matched watcher banner
# (FM_GUARD_GRACE) - do not duplicate the health predicate. Because the watcher
# touches its beacon every poll cycle, a normal fire leaves a recent beacon well
# inside grace and stays silent only when the live lock also matches. Call after
# the queue is emptied so guard never re-prints its own queued-wakes notice for
# the records this run just drained, and never let a guard hiccup change the
# drain's exit status.
