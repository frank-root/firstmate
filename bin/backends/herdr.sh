...
# Alternation, not a [❯›] bracket class: under the C locale, grep -E decomposes
# a bracket expression's multi-byte UTF-8 characters into individual bytes, and
# both glyphs share a leading 0xE2 byte with every other U+2xxx box-drawing
# character (│ ╭ ╰ ...), so the bracket form spuriously matched composer
# border rows.
