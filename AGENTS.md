...
config/crew-capability-pack  optional path to a local capability pack directory (opt-in prompt-defense preamble prepended into every ship/scout brief; PreToolUse fact-forcing hook wired into Claude ship/scout crewmate spawns only, never secondmates); LOCAL, gitignored; absent = no-op, which is the default for every installation of this shared template; inherited by secondmate homes as the literal file, same as config/crew-harness; see docs/crew-capability-pack.md
...
`config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, and `config/crew-capability-pack` are inherited into every secondmate home; `config/secondmate-harness` is not, because secondmates never spawn secondmates.
