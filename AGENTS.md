...
config/crew-capability-pack  optional path to a local capability pack directory (opt-in prompt-defense preamble and PreToolUse fact-forcing hook for Claude ship/scout crewmate and scout spawns, never secondmates); LOCAL, gitignored; absent = no-op, which is the default for every installation of this shared template; inherited by secondmate homes as the literal file, same as config/crew-harness; see docs/crew-capability-pack.md
