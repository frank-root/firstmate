...
The drain script calls that guard after emptying the queue, which avoids repeating the queued-wakes warning for records it just consumed while still warning when no live identity-matched watcher has a fresh beacon.
...
`config/crew-capability-pack` is inherited too; see [crew-capability-pack.md](crew-capability-pack.md) for the pack contract, scope, and threat model.
